...
const Show69Pattaya = () => {
  const base = toursData["show-69-pattaya"];
  if (!base) return null;
  const sheet = pattayaSheetContent["show-69-pattaya"];
  const tourData = {
    ...base,
    faqs: adultShowPattayaFaqs("69 Show Pattaya"),
    description: { ...base.description, whatToExpect: sheet.whatToExpect },
    ...(sheet.inclusions.length ? { inclusions: sheet.inclusions } : {}),
  };
  return <ThaiCityActivityTemplate tourData={tourData} config={config} contentAfterOverview={<PattayaTourItinerary tourKey="show-69-pattaya" />} />;
};
